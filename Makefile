...
LDFLAGS=-mod=mod -ldflags "-X=main.Version=$(VERSION) -X=main.Build=$(BUILD) -X=main.ProgramName=$(PROGRAMNAME)"
...
go-compile: go-get go-verify go-build-linux go-build-darwin go-build-windows
...
go-clean:
	...
	@GOPATH=$(GOPATH) GOBIN=$(GOBIN) go clean ./main.go
